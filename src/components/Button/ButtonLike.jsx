...
const ButtonLike = ({ addToFavorite }) => {
  const [activeBtn, setActiveBtn] = useState(false);

  const activateBtn = () => {
    setActiveBtn(!activeBtn);
    addToFavorite(activeBtn);
  };

  return (
    <button href="#a" onClick={activateBtn} className={['btn-reset', style.btnLike].join(' ')}>
      <svg
        width="20"
        height="20"
        viewBox="0 0 20 20"
        ...
        <path
          ...
          fill={activeBtn ? '#ff0000' : 'white'}
          stroke={activeBtn ? '#ff0000' : '#222222'}
          strokeLinecap="round"
